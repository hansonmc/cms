...
<script type="text/javascript">
	...
	function exportToExcel() {//导出到Excel
		form1.action = "${webPath}/param/export";
		form1.submit();
	}
	function importFromExcel() {//从Excel导入列表
		var text, url, webpath, winWidth, winHeight, widId, action;
		text = "";
		action = "${webPath}/param/import";
		webpath = "${webPath}";
		url = "${webPath}/common/importExcel?action="+action+"&title=系统参数Excel导入";
		winHeight = 180;
		winWidth = 650;
		widId = 'win001';
		top.ShowWin(text, url, webpath, winWidth, winHeight, widId);
