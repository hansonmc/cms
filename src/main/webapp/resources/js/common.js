				...
				_this.find("tr").hover(
						function() {
							$(this).attr("bColor",
									...
									"background-color", "#bebebe");// .css("cursor",
							// "pointer")
						},
						function() {
...
function notifyLength(obj) {
	...
	if (realLen <= maxLen) {
		...
		$("#" + spanName).html(
				"还可以输入<font color=\"#CE0000\">" + remain
						+ "</font>字母或<font color=\"#CE0000\">"
						+ (Math.round(remain / 2 - 0.5)) + "</font>汉字");
	} else {
...
function realTimeCheck(arr) {
	$.each(arr, function(key, val) {
		if (val[0] == 1) {
			$('#' + val[1]).bind('input propertychange', function() {
				notBlank(val[1], val[2]);
			});
		} else if (val[0] == 2) {
			$('#' + val[1]).bind('blur', function() {
				notBlank(val[1], val[2]);
			});
		} else if (val[0] == 3) {
			$('#' + val[1]).bind('input propertychange', function() {
...
function popupMasker() {
	...
	var top = parseInt(parseInt(document.body.scrollHeight) * 0.25)
...
function imgFile(fname) {
	if (getFile(fname) == ".jpg" || getFile(fname) == ".gif"
			|| getFile(fname) == ".jpeg" || getFile(fname) == ".png") {
		return true;
	}
	return false;
}